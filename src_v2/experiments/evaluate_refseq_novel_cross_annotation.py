...
def _run_transcript_pr_row(
    *,
    ...
    print(
        f"[telos_v2] refseq-novel-eval PR start: {test_id} {model} "
    ...
    print(
        f"[telos_v2] refseq-novel-eval PR done: {test_id} {model} "
...
def run_evaluation(
    *,
    ...
    if not cells:
        print(f"[telos_v2] refseq-novel-eval: no refseq→gencode/ensembl cells under {root}")
        return 2
    ...
    for cell_id, cell_dir in cells:
        ...
        if not gffcompare:
            print(f"[telos_v2] skip cell {cell_id}: gffcompare not found")
            continue
        tests = mapping.get("tests") or []
        if not isinstance(tests, list):
            continue
        print(f"[telos_v2] refseq-novel-eval cell: {cell_id} ({len(tests)} tests)", flush=True)
        n_done = 0
        ...
        for t in tests:
            ...
            if not asm_path.is_file() or not ref_gtf.is_file():
                print(f"[telos_v2] skip {test_id}: missing assembly or ref GTF", flush=True)
                continue

            print(
                f"[telos_v2] refseq-novel-eval test {n_done + 1}: {test_id} "
            ...
            print(
                f"[telos_v2] refseq-novel-eval {test_id}: "
                ...
                for model, ranked_name in (
                    ...
                    if not keep_ids:
                        ...
                        try:
                            ...
                            print(
                                f"[telos_v2] refseq-novel-eval PR failed: {test_id} {model}: {exc}",
                                flush=True,
                            )
                    else:
            ...
            print(f"[telos_v2] refseq-novel-eval {test_id}: site AUPR ...", flush=True)
    ...
    print(f"[telos_v2] refseq-novel-eval wrote: {reports / 'novel_eval_summary.tsv'}")
