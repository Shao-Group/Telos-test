...
Usage::
  ...
  PYTHONPATH=src_v2 python src_v2/experiments/plot_pr_curves_from_predictions.py \\
...
from telos_v2.plotting.v1_style_labels import (
...
def _plot_stacked_pr_figure(
    ...
    *,
    ...
    plt.close(fig)
    if missing:
        print(f"[telos_v2] {spec.figure_id}: {missing} missing assembler curves (subset={subset})")
    return missing == 0
...
def _plot_stacked_jaccard_bars(
    ...
    *,
    ...
    if skip:
        print("[telos_v2] skip jaccard bars (--skip-jaccard)")
        return
    ...
    if not recompute:
        ...
        if jaccard_df is not None:
            print(f"[telos_v2] jaccard: using cache {cache_path}")

    if jaccard_df is None:
        print("[telos_v2] jaccard: computing site-set similarities (slow; cached afterward)")
        ...
        if jaccard_df.empty:
            print("[telos_v2] skip jaccard bars: no site-set pairs found")
            return
        jaccard_df.to_csv(cache_path, index=False)
    elif jaccard_df.empty:
        print("[telos_v2] skip jaccard bars: empty cache")
        return
    ...
    if not panels:
        print("[telos_v2] skip jaccard bars: no TSS/TES rows")
        return
    ...
    print(f"[telos_v2] fig13 jaccard bars -> {fig_path} ({pair_label}; {sr_label} on SR)")
...
def _plot_stacked_aupr_bars(
    ...
    *,
    ...
    for site, fig_name in (("tss", "fig07_stage1_tss_aupr_bars.pdf"), ("tes", "fig08_stage1_tes_aupr_bars.pdf"), ("transcript", "fig12_transcript_pr_aupr_bars.pdf")):
        ...
        if per_row.empty:
            print(f"[telos_v2] skip {fig_name}: no {site} scores")
            continue
        panels: list[BarPanel] = []
    ...
    *,
    ...
    if write_publication:
        for spec in PUBLICATION_PR_FIGURES:
            ...
            print(f"[telos_v2] {spec.figure_id} -> {out_pdf} ({status})")
        ...
        print(f"[telos_v2] fig07/fig08 AuPR bars -> {pub_out}")
    ...
    print(f"[telos_v2] PR outputs under: {pub_out.resolve()}")
    return 0
