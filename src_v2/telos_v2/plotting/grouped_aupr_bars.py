...
Used by ``telos_v2.plotting.plot_experiments`` (outputs under ``plot_v2/``).
"""
    ...
    *,
    ...
    for panel in panels:
        ...
        for spec in metric_specs:
            ...
            if per_row.empty:
                print(f"[telos_v2] cross-annotation plot: skip {stem}/{panel} (no data)")
                continue

            ordered = order_scores_for_v1_tool_panel(per_row, panel=panel)
            if ordered.empty:
                print(f"[telos_v2] cross-annotation plot: skip {stem}/{panel} (no rows)")
                continue
            if len(ordered) != n_exp:
                print(
                    f"[telos_v2] cross-annotation plot: {stem}/{panel} has {len(ordered)} "
...
def plot_benchmark_aupr_bars(
    ...
    *,
    ...
    for spec in metric_specs:
        ...
        per_row = _extract_metric_scores(df, spec)
        if per_row.empty:
            print(f"[telos_v2] plot: skip {stem} (no data)")
            continue
    ...
    for facet_val in facets:
        ...
        for panel in panels:
            ...
            for stem, title, filters, scale_100 in metric_specs:
                ...
                if per_row.empty:
                    print(f"[telos_v2] refseq-novel plot: skip {stem}/{panel} (no data)")
                    continue

                ordered = order_scores_for_v1_tool_panel(per_row, panel=panel)
                if ordered.empty:
                    print(f"[telos_v2] refseq-novel plot: skip {stem}/{panel} (no rows)")
                    continue
                if len(ordered) != n_exp:
                    print(
                        f"[telos_v2] refseq-novel plot: {stem}/{panel} has {len(ordered)} "
                        f"rows (expected {n_exp})"
                    )
    ...
    for facet_val in facets:
        ...
        for stem, title, filters, scale_100 in metric_specs:
            ...
            if facet_val is not None and facet_col in sub.columns:
                sub = sub[sub[facet_col].astype(str) == facet_val]
            if sub.empty:
                print(f"[telos_v2] refseq-novel plot: skip {stem} (facet={facet_val})")
                continue
