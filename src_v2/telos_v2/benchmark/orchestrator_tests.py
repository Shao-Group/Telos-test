...
from telos_v2.benchmark.timing import elapsed_s, log_ts, utc_now_iso
...
def execute_benchmark_test(job: BenchmarkTestJob) -> dict[str, Any]:
    ...
    log_ts(f"[telos_v2] benchmark {i}/{job.n_tests}: predict {test_id} start")
    predict_code = run_predict(predict_io)
    predict_elapsed = elapsed_s(t0)
    log_ts(
        f"[telos_v2] benchmark {i}/{job.n_tests}: predict {test_id} end "
    ...
    if job.timing_csv is not None:
        from telos_v2.benchmark.timing import append_timing_row
