"""
CLI for experiment plots. Benchmark bar figures (PDF) go under ``plot_v2/<run_name>/``.

Usage::

  PYTHONPATH=src python -m telos_repro.plotting.plot_experiments mouse
  PYTHONPATH=src python -m telos_repro.plotting.plot_experiments tissue
  PYTHONPATH=src python -m telos_repro.plotting.plot_experiments cross-annotation
  PYTHONPATH=src python -m telos_repro.plotting.plot_experiments novel-phase-a-cross
  PYTHONPATH=src python -m telos_repro.plotting.plot_experiments refseq-novel
  PYTHONPATH=src python -m telos_repro.plotting.plot_experiments feature-importance
  PYTHONPATH=src python -m telos_repro.plotting.plot_experiments window
  PYTHONPATH=src python -m telos_repro.plotting.plot_experiments all

Or: ``telos-repro plot <id>``.
"""
...
def run_cross_annotation(root: Path, outdir: Path, pairings: tuple[str, ...]) -> None:
    ...
    for pairing in pairings:
        ...
        if sub.empty:
            print(f"[telos_repro] cross-annotation: skip {pairing} (no rows)")
            continue
        ...
        print(f"[telos_repro] cross-annotation {pairing} -> {pair_out}")
...
def run_feature_importance(run_root: Path, outdir: Path, *, top_n: int = 10) -> None:
    from telos_repro.plotting.feature_window_plots import run_feature_importance_plots
...
def run_window_sweep(root: Path, outdir: Path) -> None:
    from telos_repro.plotting.feature_window_plots import run_window_plots
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.mode == "mouse":
        ...
        print(f"[telos_repro] refseq-novel -> {out}")
        ...
        print(f"[telos_repro] feature-importance -> {out}")
    elif args.mode == "window":
        root = args.root.resolve()
        out = out_override or (DEFAULT_PLOT_V2_ROOT / root.name).resolve()
        run_window_sweep(root, out)
        print(f"[telos_repro] window sweep -> {out}")
    elif args.mode == "all":
        ...
        print(f"[telos_repro] all figures under {DEFAULT_PLOT_V2_ROOT.resolve()}")
    else:
