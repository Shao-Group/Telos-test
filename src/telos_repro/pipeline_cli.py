"""
Low-level train/predict/benchmark CLI (prefer ``telos`` + ``telos-repro``).

Defines ``argparse`` subcommands and maps parsed arguments to frozen IO dataclasses
(:mod:`telos.config_models`) and the thin handlers in :mod:`telos_repro.commands`.
"""
...
from telos_repro.commands.benchmark import run_benchmark
from telos_repro.commands.benchmark_matrix import run_benchmark_matrix
...
def _default_outdir(command: str) -> Path:
    ...
    Pattern: ``./telos_repro_<command>_<YYYYMMDD_HHMMSS>/`` (resolved absolute). Used when ``train``,
    ``predict``, or ``benchmark`` omits ``--outdir``.
    """
    stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    return (Path.cwd() / f"telos_repro_{command}_{stamp}").resolve()
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p = argparse.ArgumentParser(
        prog="telos_repro",
        description="Telos-repro pipeline: Stage I/II train, predict, and benchmark.",
    )
    ...
    train.add_argument(
        ...
        type=Path,
        default=None,
        help="Output directory (default: ./telos_repro_train_<timestamp>).",
    )
    train.add_argument(
        "--config",
        type=Path,
    ...
    predict.add_argument(
        ...
        type=Path,
        default=None,
        help="Output directory (default: ./telos_repro_predict_<timestamp>).",
    )
    predict.add_argument(
        "--config",
        type=Path,
    ...
    bench.add_argument(
        ...
        type=Path,
        default=None,
        help="Output directory (default: ./telos_repro_benchmark_<timestamp>).",
    )
    ...
    bmat.add_argument(
        ...
        type=Path,
        default=None,
        help="Telos stage1 YAML (default: configs/stage1.defaults.yaml).",
    )
