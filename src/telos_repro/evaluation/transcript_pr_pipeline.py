...
from telos_repro.backends.gtfcuff import run_gtfcuff_auc, run_gtfcuff_roc
from telos.backends.gtfformat import run_filter_chrom, run_update_transcript_cov
from telos.gtf_attributes import parse_transcript_id
from telos.models.chrom_split import seqnames_on_validation_split_from_gtf
...
def write_predictions_for_update_cov(ranked_tsv: Path, out_tsv: Path, *, score_col: str = "pred_prob") -> None:
    """
    Write TSV consumed by :func:`telos.backends.gtfformat.run_update_transcript_cov`: header + rows.
...
def parse_gtfcuff_roc_stdout(text: str) -> tuple[np.ndarray, np.ndarray]:
    """
    Parse recall/precision pairs from :func:`telos_repro.backends.gtfcuff.run_gtfcuff_roc` text output.
...
def _apply_pr_chrom_filter(
    *,
    ...
    Returns:
        ...
        auto mode derives lists from :func:`~telos.models.chrom_split.seqnames_on_validation_split_from_gtf`
    ...
    if use_explicit:
        ...
        run_filter_chrom(assembly_gtf, chrom_file, asm_out)
        run_filter_chrom(ref_gtf, chrom_file, ref_out)
    ...
    try:
        if asm_names:
            ...
            asm_out = work_dir / "query_validation_chrom.gtf"
            run_filter_chrom(assembly_gtf, list_asm, asm_out)
            meta["pr_filtered_assembly_gtf"] = str(asm_out.resolve())
        else:
        ...
        if ref_names:
            ...
            ref_out = work_dir / "ref_validation_chrom.gtf"
            run_filter_chrom(ref_gtf, list_ref, ref_out)
            meta["pr_filtered_ref_gtf"] = str(ref_out.resolve())
        else:
...
def run_transcript_pr_benchmark(
    ...
    **Ordered steps**
    ...
    run_update_transcript_cov(assembly_eff, pred_tsv, updated_gtf)
