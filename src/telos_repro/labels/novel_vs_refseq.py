...
from telos_repro.benchmark.stage1_metrics import safe_aupr
from telos.gtf_attributes import parse_transcript_id
from telos.labels.site_labels import (
...
def get_or_build_refseq_index(refseq_gtf: Path, cache_path: Path) -> RefSeqTranscriptIndex:
    ...
    if cache_path.is_file() and cache_path.stat().st_mtime >= refseq_gtf.stat().st_mtime:
        print(f"[telos_repro] refseq-novel: loading transcript index cache {cache_path}", flush=True)
        return load_refseq_index(cache_path)
    print(f"[telos_repro] refseq-novel: building transcript index from {refseq_gtf} ...", flush=True)
    idx = build_refseq_transcript_index(refseq_gtf)
    save_refseq_index(idx, cache_path)
    print(
        f"[telos_repro] refseq-novel: index built ({len(idx.by_boundary)} boundary keys) -> {cache_path}",
        flush=True,
    )
    return idx
...
def get_or_build_refseq_endpoint_sites(refseq_gtf: Path, cache_path: Path) -> pd.DataFrame:
    refseq_gtf = refseq_gtf.resolve()
    if cache_path.is_file() and cache_path.stat().st_mtime >= refseq_gtf.stat().st_mtime:
        print(f"[telos_repro] refseq-novel: loading endpoint sites cache {cache_path}", flush=True)
        return pd.read_csv(cache_path, sep="\t")
    print(f"[telos_repro] refseq-novel: extracting RefSeq TSS/TES from {refseq_gtf} ...", flush=True)
    sites = refseq_endpoint_sites(refseq_gtf)
    cache_path.parent.mkdir(parents=True, exist_ok=True)
    sites.to_csv(cache_path, sep="\t", index=False)
    print(f"[telos_repro] refseq-novel: cached {len(sites)} RefSeq endpoint sites", flush=True)
    return sites
