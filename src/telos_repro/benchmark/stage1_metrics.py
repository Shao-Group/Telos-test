...
def evaluate_stage1_test_aupr(
    ...
    *,
    ...
    coordinates (after chromosome normalization), labels via :func:`~telos.labels.site_labels.label_sites_by_proximity`,
    ...
    for st in ("TSS", "TES"):
        ...
        if sub.empty:
            ...
            if n_before > 0:
                print(
                    f"[telos_repro] benchmark Stage I test AUPR: no rows for {st} after cov coordinate join "
