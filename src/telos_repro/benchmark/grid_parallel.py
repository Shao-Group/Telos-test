...
from telos_repro.benchmark.matrix import build_benchmark_yaml_mapping
from telos_repro.benchmark.shared_train import (
    apply_train_reuse,
    shared_models_ready,
    shared_train_dir,
    shared_train_lock,
)
from telos_repro.benchmark.util import as_path
...
def cap_parallel_tests(
    *,
    ...
    if capped < max_parallel_tests:
        print(
            f"[telos_repro] capped max_parallel_tests {max_parallel_tests} -> {capped} "
...
def run_shared_trains_parallel(
    ...
    *,
    ...
    print(f"[telos_repro] shared-train phase: {len(jobs)} axes, max_workers={workers}", flush=True)
    ...
    with ProcessPoolExecutor(max_workers=workers) as pool:
        ...
        for fut in as_completed(futures):
            ...
            print(f"[telos_repro] shared-train {axis_id}: {status} (code={code})", flush=True)
    return results
...
def _worker_run_benchmark_cell(job: BenchmarkCellJob) -> dict[str, str]:
    from telos_repro.benchmark.matrix import benchmark_mapping_to_yaml_text
...
def run_benchmark_cells_parallel(
    ...
    *,
    ...
    if not jobs:
        return []
    workers = max(1, min(max_workers, len(jobs)))
    print(f"[telos_repro] benchmark-cell phase: {len(jobs)} cells, max_workers={workers}", flush=True)
    ...
    with ProcessPoolExecutor(max_workers=workers) as pool:
        ...
        for fut in as_completed(futures):
            ...
            rows.append(row)
            print(
                f"[telos_repro] cell {done}/{len(jobs)}: {job.combo_id} -> {row['status']} (code={row['exit_code']})",
                flush=True,
            )
    return rows
