...
def _rebase_bundle_path(path: Path, bundles_root: Path) -> Path:
    """
    Prefer ``path`` if it exists; otherwise remap ``.../bundles/<rest>`` under ``bundles_root``.

    data-cp manifests often still point at absolute ``Telos-test/data/bundles/...`` paths that
    were moved; keep manifests read-only and rewrite at load time.
    """
    path = Path(path)
    if path.is_file():
        return path.resolve()
    ...
    if "bundles" in parts:
        ...
        cand = (bundles_root.joinpath(*parts[idx + 1 :])).resolve()
        if cand.is_file():
            return cand
    return path


def _manifest_bam_ref(
    manifest: dict[str, Any], *, bundles_root: Path | None = None
) -> tuple[Path, Path]:
    """
    ...
    if bundles_root is not None:
        bam = _rebase_bundle_path(bam, bundles_root)
        # ref_gtf usually under genome/, not bundles/; only rebase if missing and looks bundled
        if not ref_gtf.is_file():
            ref_gtf = _rebase_bundle_path(ref_gtf, bundles_root)
    return bam, ref_gtf
...
def _assemblies_with_gffcompare_tmap(
    manifest: dict[str, Any], *, bundles_root: Path | None = None
    ...
    for ass in manifest.get("assemblies") or []:
        ...
        try:
            ...
            continue
        if bundles_root is not None:
            gtf = _rebase_bundle_path(gtf, bundles_root)
            tmap = _rebase_bundle_path(tmap, bundles_root)
...
def _stringtie_paths(
    ...
    """
    bam, ref_gtf = _manifest_bam_ref(manifest, bundles_root=bundles_root)
    for ass in manifest.get("assemblies") or []:
        if not isinstance(ass, dict):
            continue
...
def _train_pool_paths(
    ...
    gtfs: list[Path] = []
    tmaps: list[Path] = []
    for aid, gtf, tmap in _assemblies_with_gffcompare_tmap(manifest, bundles_root=bundles_root):
        if aid == "stringtie":
            continue
        gtfs.append(gtf)
        tmaps.append(tmap)
...
def build_benchmark_yaml_mapping(
    *,
    ...
    tbam, tgtf, tref, ttmap = _stringtie_paths(train_mf, bundles_root=bundles_root)
    pool_gtfs, pool_tmaps = _train_pool_paths(train_mf, bundles_root=bundles_root)
    ...
    for sid in test_ids:
        ...
        bam, ref_gtf = _manifest_bam_ref(mf, bundles_root=bundles_root)
        per_asm = _assemblies_with_gffcompare_tmap(mf, bundles_root=bundles_root)
        if not per_asm:
            raise ValueError(
    ...
    return {
        "generated_by": "telos_repro.benchmark.matrix",
        "schema_version": 3,
        "train": train_block,
        "tests": tests,
...
def run_benchmark_matrix(
    *,
    ...
    2. Picks Stage I config from ``stage1_config`` or
       :func:`~telos_repro.config_loader.default_stage1_config_path`; returns exit code ``2`` if missing.
    ...
    5. Calls :func:`~telos_repro.benchmark.orchestrator.run_benchmark` with ``BenchmarkIO`` pointing at
       that file and ``outdir``.

    Returns:
        Process exit code from :func:`~telos_repro.benchmark.orchestrator.run_benchmark`, or ``2`` on
    ...
    if not stage1.is_file():
        print(f"[telos_repro] benchmark-matrix: stage1 config not found: {stage1}")
        return 2
    ...
    try:
        ...
        print(f"[telos_repro] benchmark-matrix: {exc}")
        return 2
    ...
    header = (
        "# Auto-generated by: python -m telos_repro.cli benchmark-matrix (...)\n"
    ...
    print(f"[telos_repro] benchmark-matrix: wrote {cfg_path}")
