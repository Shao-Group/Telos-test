...
from telos_repro.benchmark.grid_parallel import (
    BenchmarkCellJob,
...
def run_cross_annotation_benchmarks(
    *,
    ...
    if not stage1.is_file():
        print(f"[telos_repro] cross-annotation: stage1 config not found: {stage1}")
        return 2
    ...
    if not cell_jobs:
        print("[telos_repro] cross-annotation: no matrix cells to run.")
        return 2
    if only_same_annotation:
        print(
            f"[telos_repro] cross-annotation: running {len(cell_jobs)} same-annotation cells only "
    ...
    if use_parallel_grid:
        ...
        if bad_train:
            print(f"[telos_repro] cross-annotation: shared-train failed for: {', '.join(bad_train)}")
        if max_parallel_cells > 1:
    ...
    print(f"[telos_repro] cross-annotation index: {run_index}")
...
def _run_cells_sequential(
    ...
    *,
    ...
    for i, job in enumerate(cell_jobs, start=1):
        print(f"[telos_repro] cross-annotation {i}/{n}: {job.combo_id}", flush=True)
        ...
        try:
            ...
            print(f"[telos_repro] cross-annotation setup failed: {exc}")
            continue
