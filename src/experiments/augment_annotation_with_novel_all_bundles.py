...
from telos_repro.benchmark.matrix import resolve_bundles_root
...
def run(args: argparse.Namespace) -> int:
    ...
    for i, (ref_id, modality, sample, mf) in enumerate(manifests, start=1):
        ...
        print(f"[telos_repro] augment-all {i}/{len(manifests)}: {rel}")
        try:
    ...
    print("[telos_repro] augment-all complete")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        ...
        type=Path,
        default=None,
        help="Stage1 config path; defaults to telos_repro default stage1 config",
    )
    p.add_argument(
        "--out-root",
        type=Path,
