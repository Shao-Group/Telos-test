...
from telos_repro.benchmark.matrix import ANNOTATION_TO_REF_ID
from telos_repro.evaluation.transcript_pr_pipeline import run_gffcompare
...
def run(
    *,
    ...
    for i, row in enumerate(ref_rows, start=1):
        ...
        print(f"[telos_repro] augmented tmaps {i}: {rel}")
    ...
    print("[telos_repro] augmented tmaps index complete")
