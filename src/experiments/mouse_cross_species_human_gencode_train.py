...
Usage (from repo root)::
    ...
    PYTHONPATH=src python src/experiments/mouse_cross_species_human_gencode_train.py \\
...
def run_all(
    *,
    ...
    for dt in data_types:
        ...
        print(f"[telos_repro] mouse cross-species: {run_id}")
        try:
            ...
            print(f"[telos_repro] setup failed: {exc}")
            continue
    ...
    with index_csv.open("w", newline="", encoding="utf-8") as fh:
        ...
        w.writeheader()
        w.writerows(rows)
    print(f"[telos_repro] run index: {index_csv}")
...
def main() -> int:
    ...
    p.add_argument(
        ...
        type=Path,
        default=None,
        help="Stage I YAML (default: telos_repro default_stage1_config_path()).",
    )
    p.add_argument(
        "--data-types",
        nargs="+",
